...
POWER_RULES = {}      # power rule id -> title
for f in glob.glob(GM + 'rules/magic/powers/*.md'):
    t = open(f, encoding='utf-8').read()
    rid = re.search(r'^id: "(.+)"$', t, re.M)
    ttl = re.search(r'^title: "(.+)"$', t, re.M)
    if rid and ttl:
        POWER_RULES[rid.group(1)] = ttl.group(1)
...
powers = collections.defaultdict(set)   # power name -> {who}
power_rule = {}                         # power name -> rule id claimed by a sheet
...
for f in glob.glob(PKG + 'characters/**/*.json', recursive=True) + \
    ...
    s = d.get('spells')
    who = d.get('name') or os.path.basename(f)
    if isinstance(s, dict):
        for lst in s.values():
            for n in lst:
                sheets[n].add(who)
    for pw in d.get('powers') or []:
        powers[pw['name']].add(who)
        power_rule[pw['name']] = pw.get('rule')
...
for name in sorted(powers):
    rid = power_rule.get(name)
    preg[name] = {"source": "mythras-powers-chassis", "licence": "ours",
                  "rule": rid if rid in POWER_RULES else None,
                  "used_by": sorted(powers[name])}
...
print(f'{len(preg)} powers on sheets, all ours')

gap = sorted(n for n, v in {**reg, **preg}.items() if not v["rule"])
if gap:
    print(f'\n{len(gap)} of {len(reg) + len(preg)} have NO RULE in the graph:')
    for n in gap:
        v = {**reg, **preg}[n]
    ...
    print('\nEvery spell and power on every sheet has a rule behind it.')
